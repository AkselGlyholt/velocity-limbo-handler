...
public final class ConsumerPlugin {
    ...
    public HoldResult pause(String serverName) {
        if (controller == null) {
            throw new IllegalStateException("VLH controller is not available before proxy initialization");
        }
        return controller.holdServer(
                serverName,
                new HoldRequest("consumer smoke test", Duration.ofMinutes(5))
        );
    }
    ...
    public HoldReleaseStatus resume(UUID leaseId) {
        ...
        return controller.releaseHold(leaseId);
    }
}
